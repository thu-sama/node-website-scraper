...
module.exports = function generateFilename (resource, options, occupiedFileNames) {
	...
	var directory = getDirectoryByExtension(resource.mimeType, extension, options);
...
function getDirectoryByExtension (mimeType, extension, options) {
	return _(options.subdirectories)
		.filter(function matchesExtension (directory) { return _.includes(directory.extensions, extension) || _.includes(directory.mime_types, mimeType); })
